...
/// This module implements Into<jsonrpc_core::IoHandler> for APIForeign and APIOwner
/// APIOwner implementation in progress
mod jsonrpc_api {
	use crate::keychain::Keychain;
	use crate::libwallet::api::APIForeign;
	use crate::libwallet::types::{BlockFees, NodeClient, WalletBackend};
	...
	//
	// Baring further disscussion. Option 2 will be used, as it is simplest to implement and
	// does not potentially leak sensitive information through structured errors.

	impl<W, C, K> Into<IoHandler> for APIForeign<W, C, K>
	where
		W: ?Sized + WalletBackend<C, K> + 'static,
		C: NodeClient + 'static,
		K: Keychain + 'static,
	{
		fn into(self) -> IoHandler {
			let mut io = IoHandler::new(); // Value to be returned.

			// each closure gets its own copy of the API object; copies are cheap
			let api_copy = self.clone();
			add_rpc_method(
				&mut io,
				"build_coinbase",
				&["block_fees"],
				move |mut args: Vec<Value>| {
					...
					let unreachable_argnum_err =
						|| InvalidArgs::WrongNumberOfArgs { expected, actual };
					...
					let arg0: BlockFees = serde_json::from_value(next_arg).map_err(|_| {
	...
	mod test {
		...
		use crate::libwallet::types::WalletBackend;
		use crate::util;
		use crate::{HTTPNodeClient, LMDBBackend, WalletConfig};
		use jsonrpc_core::IoHandler;
		use std::sync::Arc;

		fn create_api_both() -> (
			APIOwner<WalletBackend<HTTPNodeClient, ExtKeychain>, HTTPNodeClient, ExtKeychain>,
			APIForeign<WalletBackend<HTTPNodeClient, ExtKeychain>, HTTPNodeClient, ExtKeychain>,
		) {
		...
		fn create_api_foreign() -> IoHandler {
			create_api_both().1.into()
		}

		#[test]
		fn api_foreign_create_keybase() {
			let api = create_api_foreign();
			api.
		}
	}
}
